""" HELPS TO GET THE INDEX OF FIRST OCCURENCES OF KEY - IF KEY PRESENT
    ElSE THE INDEX OF GREATER ELEMENT LESS THAN KEY
"""
def lowerBoundBinaryIterative(arr, n, key):
...
def lowerBoundBinaryRecursion(arr, low, high, key, ans):
    ...
    if low > high:
        ...
        if arr[mid] >= key:
            ans = mid
            return lowerBoundBinaryRecursion(arr, low, mid - 1, key, ans)
        else:
            return lowerBoundBinaryRecursion(arr, mid + 1, high, key, ans)
        

lst = [1, 3, 5, 9, 9, 12, 14, 14, 14, 16, 17]
key = int(input("Enter key element to search : "))

print(lowerBoundBinaryIterative(lst, len(lst), key))

print(lowerBoundBinaryRecursion(lst, 0, len(lst) - 1, key, len(lst)))
